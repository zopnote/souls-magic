...
public class WeatherAttack {
  public WeatherAttack(Player player, boolean pvp) {
    ...
    for (int count = 0; count < 17; count++) {
      ...
      world.spawnParticle(Particle.DRIPPING_DRIPSTONE_WATER, position, 1);
  ...
  private void damageProcess(Collection<LivingEntity> targetEntities, Player player, boolean pvp, Collection<Player> nearbyPlayers) {
    for (LivingEntity targetEntity : targetEntities) {
      ...
      for (Player nearbyPlayer : nearbyPlayers) {
        nearbyPlayer.spawnParticle(Particle.CAMPFIRE_SIGNAL_SMOKE, targetEntity.getLocation(), 7);
        nearbyPlayer.playSound(targetEntity.getLocation(), Sound.BLOCK_ANVIL_STEP, 1.0f, 0.1f);
      }
    }
  }
}
